...
define(["jquery", "handlebars", "underscore", "backbone", "org/forgerock/openidm/ui/common/dashboard/widgets/AbstractWidget", "org/forgerock/commons/ui/common/main/EventManager", "backgrid", "org/forgerock/commons/ui/common/util/BackgridUtils"], function ($, handlebars, _, Backbone, AbstractWidget, eventManager, Backgrid, BackgridUtils) {
    var widgetInstance = {},
        ...
        widgetRender: function widgetRender(args, callback) {
            ...
            this.events["click .widget-body a"] = function (e) {
                e.preventDefault();
                var parent = $(event.target).parents('.card'),
                    index = this.$el.find(".card").index(parent),
                    href = handlebars.Utils.escapeExpression(this.data.cards[index].href);

                window.location.href = window.location.protocol + "//" + window.location.host + window.location.pathname + href;
            };
        ...
        customSettingsSave: function customSettingsSave(dialogRef, widget) {
            ...
            _.forEach(this.model.QuickObjectCollection.models, function (model) {
                widget.cards.push({
                    ...
                    "href": handlebars.escapeExpression(model.attributes.href)
                });
            });
