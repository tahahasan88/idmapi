    ...
    var EditResourceView = AbstractView.extend({
                    ...
                    _this2.parentRender(function () {
                        ...
                        ValidatorsManager.bindValidators(_this2.$el.find("#resource"), [_this2.data.objectType, _this2.objectName, _this2.objectId || "*"].join("/"), function () {
                            ...
                            if (!_this2.data.newObject) {
                                ...
                                // schema.id is optional and can be any of audit like auditaccess,auditrecon
                                var eventId = schema && schema.id && schema.id.startsWith("audit") ? resource.eventId : resource._id;
                                _this2.linkedView.render({ id: eventId, resourcePath: _this2.data.objectType + "/" + _this2.objectName + "/" });
                            }
        ...
        setupEditor: function setupEditor(resource, schema) {
            ...
            var propCount = 0,
                ...
                viewableVirtualProperties = [];

            this.oldObject = $.extend(true, {}, filteredObject);
            this.relationshipArrayProperties = [];
            ...
            if (schema.order) {
                _.forEach(schema.order, function (prop) {
                    ...
                    } else if (schema.properties[prop].viewable && schema.properties[prop].isVirtual) {
                        viewableVirtualProperties.push(prop);
                    } else if (schema.properties[prop].viewable && !_.has(filteredObject, prop)) {
                        filteredObject[prop] = null;
                    }
                    // if this is a new object and the property has a default set the field with that value
                    if (_this3.data.newObject && schema.properties[prop].default) {
                        filteredObject[prop] = schema.properties[prop].default;
                    }
                    // if this is a relationship array property add it to this.relationshipArrayProperties
                    if (schema.properties[prop].type === "array" && schema.properties[prop].items.typeRelationship) {
                        _this3.relationshipArrayProperties.push(prop);
                    }
                });
            }
            ...
            // disable virtual properties
            viewableVirtualProperties.forEach(function (virtualProp) {
                var propEditor = _this3.editor.getEditor("root." + virtualProp);
                if (propEditor) {
                    propEditor.disable();
                }
            });
        ...
        generateChangedFieldsList: function generateChangedFieldsList() {
            ...
            _.forEach(newValue, function (val, key) {
                ...
                // this prevents a form without temporal constraints turned on from triggering pending changes
                if (key === 'temporalConstraints' && val === undefined) {
                    val = [];
                }

                if (!_this4.oldObject[key] && hasVal && !relationshipType || !relationshipType && _this4.oldObject[key] && !_.isEqual(_this4.oldObject[key], val) || relationshipType && _this4.data.schema.properties[key].type !== "array" && hasVal && !_.isEqual(JSON.parse(val), _this4.oldObject[key])) {
        ...
        getFormValue: function getFormValue() {
            ...
            _.forEach(formVal, function (v, k) {
                ...
                // Look for number type fields whose actual value is an empty string and set them to undefined
                // formVal[key] will be 0 on number types because JSONEditor.getValue() sets them to 0 by default
                if (_.isNumber(formVal[k]) && formVal[k] === 0 && _this6.$el.find("#0-root-" + k).length === 1 && _this6.$el.find("#0-root-" + k)[0].value === "") {
                    formVal[k] = undefined;
                }
            });
            ...
            // remove relatiohship array type properties from formVal
            _.forEach(this.relationshipArrayProperties, function (relationshipArrayProperty) {
                delete formVal[relationshipArrayProperty];
            });
            return formVal;
        },
        ...
        save: function save(e, callback) {
            ...
            var formVal = CommonUIUtils.replaceEmptyStringWithNull(this.getFormValue()),
                ...
                if (xhr.responseJSON && xhr.responseJSON.code === 400 && xhr.responseJSON.message) {
                    messagesManager.messages.addMessage({ "type": "error", "message": xhr.responseJSON.message });
                }
                if (xhr.responseJSON && xhr.responseJSON.code === 403 && xhr.responseJSON.message) {
                    if (xhr.responseJSON.detail && xhr.responseJSON.detail.failedPolicyRequirements) {
                        var errMsg = PolicyValidatorsManager.failedPolicyRequirementObjectsToStrings(xhr.responseJSON.detail.failedPolicyRequirements);
                        errMsg.unshift(xhr.responseJSON.message); // errMsg is an array, add message to the beginning
                        errMsg = errMsg.join("<br />");
                        messagesManager.messages.addMessage({ "type": "error", "message": errMsg, "escape": false });
                    }
                }
            };
            ...
            if (this.data.newObject) {
                ...
                ResourceDelegate.createResource(this.data.serviceUrl, formVal._id, formVal, successCallback, errorCallback);
                ...
                ResourceDelegate.patchResourceDifferences(this.data.serviceUrl, { id: this.oldObject._id, rev: this.oldObject._rev }, this.oldObject, _.assignIn({}, this.oldObject, formVal), successCallback, errorCallback);
            } else {
                ResourceDelegate.updateResource(this.data.serviceUrl, this.oldObject._id, formVal, successCallback, errorCallback);
            }
        },
            ...
            _convertField = function convertField(prop) {
                ...
                relationshipDisplay.click(function (e) {
                    ...
                    if ($(e.target).attr("id") === removeButtonId || $(e.target).closest(".removeRelationshipButton").attr("id") === removeButtonId) {
                        ...
                        relationshipDisplay.remove();
                        _convertField(prop);
                        // When a singleton relationship is removed we want to always save the form
                        // otherwise if we remove it and "update" the relationship an incorrect
                        // patch request will be sent and an 'Invalid field for singleton relationship'
                        // error will be displayed. See OPENIDM-17192
                        _this.save();
                    }
                });
